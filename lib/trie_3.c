...
static uint32_t
trie_alloc_16(struct trie_data *td, uint32_t size)
{
    if (!size) size = 2;
    size = (size + 1) & ~1;
    if (!size) return 0;
    ...
    if ((uint16_t) new_size != new_size) {
        fprintf(stderr, "trie_alloc_16: out of memory\n");
        abort();
    }
        ...
        unsigned char **strs)
{
    ...
    uint32_t node_offset = trie_alloc_16(td, sizeof(struct trie_node_16));
    struct trie_node_16 *node = TRIE_NODE_16(td, node_offset);
    uint32_t mem;
    ...
    if (low + 1 == high) {
        ...
        // a -> node: a
        if (len > 1) {
            mem = trie_alloc_16(td, len);
            node = TRIE_NODE_16(td, node_offset);
            node->prefix_offset = mem;
            memcpy(TRIE_PREFIX_16(td, node_offset), str + index, len - 1);
        }
        c = str[index + len - 1];
        mem = trie_alloc_16(td, sizeof(uint16_t));
        node = TRIE_NODE_16(td, node_offset);
        node->children_offset = mem;
        node->low_c = c;
        node->high_c = c;
        mem = make_trie_rec(td, low, high, index + len, sorted_idx, strs);
        TRIE_CHILD_16(td, node_offset, 0) = mem;
        return node_offset;
    }
    ...
    if (pfx_len > 0) {
        ...
        if (!c) {
            // cases 1, 3
            if (pfx_len > 1) {
                mem = trie_alloc_16(td, pfx_len);
                node = TRIE_NODE_16(td, node_offset);
                node->prefix_offset = mem;
                memcpy(TRIE_PREFIX_16(td, node_offset), str + index, pfx_len - 1);
            }
        ...
        mem = trie_alloc_16(td, pfx_len + 1);
        node = TRIE_NODE_16(td, node_offset);
        node->prefix_offset = mem;
        memcpy(TRIE_PREFIX_16(td, node_offset), str + index, pfx_len);
        index += pfx_len;
    }
    ...
    if (with_cmap_size < no_cmap_size) {
        // cmap on
        mem = trie_alloc_16(td, high_c - low_c + 1);
        node = TRIE_NODE_16(td, node_offset);
        node->cmap_offset = mem;
        mem = trie_alloc_16(td, (ccnt + 1) * sizeof(uint16_t));
        node = TRIE_NODE_16(td, node_offset);
        node->children_offset = mem;
        ...
        while (low_ind < high) {
            ...
            TRIE_CMAP_16(td, node_offset, c - low_c) = subind;
            mem = make_trie_rec(td, low_ind, high_ind, index + 1, sorted_idx, strs);
            TRIE_CHILD_16(td, node_offset, subind) = mem;
            low_ind = high_ind;
        }
    } else {
        mem = trie_alloc_16(td, (high_c - low_c + 1) * sizeof(uint16_t));
        node = TRIE_NODE_16(td, node_offset);
        node->children_offset = mem;
        uint32_t low_ind = low, high_ind = low;
        while (low_ind < high) {
            ...
            mem = make_trie_rec(td, low_ind, high_ind, index + 1, sorted_idx, strs);
            TRIE_CHILD_16(td, node_offset, c - low_c) = mem;
            low_ind = high_ind;
        }
    }
    return node_offset;
...
struct trie_data *
trie_compile_16(size_t count, unsigned char **strs)
{
...
struct trie_data *
trie_compile_f_16(unsigned char *(*index_func)(void *cntx, unsigned int index), void *cntx)
{
    uint32_t count = 0;
    while (1) {
...
void
trie_dump_16(struct trie_data *trie, FILE *fout)
{
...
void
trie_generate_c_16(struct trie_data *trie, const char *varname, FILE *fout)
{
