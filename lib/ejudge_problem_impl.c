...
find_profile(
        const struct language_info *lang,
        ...
        int normalization)
{
    ...
    if (ejudge_problem_read_full_file(log_f, path, &data, &size) < 0) {
        ...
        size_t *p_size)
{
    ...
    for (size_t i = 0; i < cc.path_u; ++i) {
        ...
        if (ejudge_problem_read_full_file(log_f, cc.paths[i], &fd, &fz) < 0) {
