...
compare_runs(FILE *fout, int run_id1, int run_id2)
{
  ...
  if (info1.language <= 0 || info1.language > max_lang
      || !langs[info1.language] || langs[info1.language]->binary) {
    errcode = -SRV_ERR_BAD_RUN_ID;
    goto cleanup;
  }
  ...
  if (info2.language <= 0 || info2.language > max_lang
      || !langs[info2.language] || langs[info2.language]->binary) {
    errcode = -SRV_ERR_BAD_RUN_ID;
    goto cleanup;
  }
  ...
  tmpfile1 = alloca(strlen(global->diff_work_dir) + 65);
  sprintf(tmpfile1, "%s/%s", global->diff_work_dir, par1);
  snprintf(par2, sizeof(par2), "d2-%06d", run_id2);
  tmpfile2 = alloca(strlen(global->diff_work_dir) + 65);
  sprintf(tmpfile2, "%s/%s", global->diff_work_dir, par2);
  tmpfile3 = alloca(strlen(global->diff_work_dir) + 65);
  ...
  task_AddArg(tsk, global->diff_path);
  task_AddArg(tsk, "-u");
  task_AddArg(tsk, par1);
  task_AddArg(tsk, par2);
  task_SetPathAsArg0(tsk);
  task_ClearEnv(tsk);
  task_SetWorkingDir(tsk, global->diff_work_dir);
