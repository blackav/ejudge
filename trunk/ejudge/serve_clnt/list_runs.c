...
int
serve_clnt_list_runs(int sock_fd, int out_fd, int cmd,
                     int user_id, int contest_id, int locale_id,
                     unsigned int flags,
                     unsigned char const *form_start)
{
  struct prot_serve_pkt_list_runs *out;
  struct prot_serve_packet *in;
  int out_size, in_size, form_start_len, r;
  int pipe_fd[2], pass_fd[2];
  unsigned char c;

  if (sock_fd < 0) return -SRV_ERR_NOT_CONNECTED;
  if (cmd != SRV_CMD_LIST_RUNS && cmd != SRV_CMD_LIST_CLARS)
    return -SRV_ERR_PROTOCOL;

  if (pipe(pipe_fd) < 0) {
    err("serve_clnt_list_runs: pipe() failed: %s", os_ErrorMsg());
  ...
  form_start_len = strlen(form_start);
  out_size = sizeof(*out) + form_start_len;
  out = alloca(out_size);
  ...
  out->flags = flags;
  out->form_start_len = form_start_len;
  memcpy(out->data, form_start, form_start_len);
  ...
  if (in->id != SRV_RPL_OK) {
    close(pipe_fd[0]);
    xfree(in);
    err("serve_clnt_list_runs: unexpected reply: %d", in->id);
    return -SRV_ERR_PROTOCOL;
  }
  xfree(in);
