                               ...
                               unsigned int *p_val)
{
  ...
  if (!vb) return -1;
  if (vb[0] == '-') fatal_CF("minus sign before uint32 value in correct");
  errno = 0;
  x = strtoul(vb, &ep, 10);
  if (*ep) fatal_CF("cannot parse uint32 value for %s from correct", name);
  if (errno) fatal_CF("uint32 value %s from correct is out of range", name);
  *p_val = x;
  return 1;
}
