...
int checker_main(int argc, char **argv)
{
  ...
  while (1) {
    ...
    if (checker_read_team_unsigned_int(buf, 0, &team_ans) < 0) {
      fatal_WA("Too few numbers in the team output");
    }
    if (corr_ans != team_ans)
      fatal_WA("Answers differ: %s: team: %u, corr: %u", buf, team_ans, corr_ans);
  }
  if (checker_read_team_unsigned_int("x", 0, &team_ans) >= 0) {
