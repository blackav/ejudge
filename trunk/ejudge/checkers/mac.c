...
static unsigned short mac_to_ucs2_table[256] =
{
...
int
checker_mac_to_ucs2(int c)
{
  return mac_to_ucs2_table[c & 0xff];
}

unsigned short *
checker_mac_to_ucs2_buf(const char *in, unsigned short *out, size_t size)
{
  const unsigned char *p = (const unsigned char*) in;
  unsigned short *q = out;

  for (; size; size--, q++, p++) *q = mac_to_ucs2_table[*p];
  return out;
}
