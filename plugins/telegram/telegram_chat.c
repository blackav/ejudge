...
telegram_chat_parse_bson(struct _bson *bson)
{
    ...
    while (bson_cursor_next(bc)) {
        ...
        if (!strcmp(key, "_id")) {
            ...
            if (ej_bson_parse_string(bc, "title", &tc->type) < 0) goto cleanup;
        } else if (!strcmp(key, "username")) {
            if (ej_bson_parse_string(bc, "username", &tc->type) < 0) goto cleanup;
        } else if (!strcmp(key, "first_name")) {
            if (ej_bson_parse_string(bc, "first_name", &tc->type) < 0) goto cleanup;
        } else if (!strcmp(key, "last_name")) {
            if (ej_bson_parse_string(bc, "last_name", &tc->type) < 0) goto cleanup;
        }
    }
    bson_cursor_free(bc);
    return tc;
