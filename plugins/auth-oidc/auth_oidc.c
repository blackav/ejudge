...
{
    struct auth_oidc_state *state = (struct auth_oidc_state*) data;
    ...
    for (struct xml_tree *p = tree->first_down; p; p = p->right) {
        ...
        } else if (!strcmp(p->name[0], "configuration_endpoint")) {
            if (xml_leaf_elem(p, &state->configuration_endpoint, 1, 0) < 0) return -1;
        } else if (!strcmp(p->name[0], "scope")) {
            if (xml_leaf_elem(p, &state->scope, 1, 0) < 0) return -1;
        }
    }
...
static int
fetch_oidc_endpoints(struct auth_oidc_state *state)
{
    char *page_text = NULL;
    size_t page_size = 0;
    FILE *file = NULL;
 ...
 */
static void
packet_handler_auth_oidc(int uid, int argc, char **argv, void *user)
{
    struct auth_oidc_state *state = (struct auth_oidc_state*) user;
    ...
    fprintf(post_f, "&redirect_uri=%s/S4", url_armor_buf(&ab, state->redirect_uri));
    ...
    // parse payload of JWT
    {
        ...
        if (err) {
            error_message = "invalid oidc json: base64u payload decode error";
            goto done;
        }
        jwt_payload[len] = 0;
    }
...
static void
queue_packet_handler_auth_oidc(int uid, int argc, char **argv, void *user)
{
    struct auth_oidc_state *state = (struct auth_oidc_state*) user;
    state->bi->enqueue_action(state->bd, packet_handler_auth_oidc, uid, argc, argv, user);
}
...
start_thread_func(void *data)
{
    ...
    state->set_command_handler_func(state->set_command_handler_data,
                                    "auth_oidc",
                                    queue_packet_handler_auth_oidc,
                                    data);
        ...
        const unsigned char *extra_data)
{
    ...
    fprintf(url_f, "&redirect_uri=%s/S4", url_armor_buf(&ab, state->redirect_uri));
    fprintf(url_f, "&state=%s", ebuf);
    if (state->scope && state->scope[0]) {
        fprintf(url_f, "&scope=%s", url_armor_buf(&ab, state->scope));
    } else {
        ...
        const unsigned char *code)
{
    ...
    if (state->send_job_handler_func) {
        unsigned char *args[] = { "auth_oidc", oas2.request_id, oas2.request_code, NULL };
...
struct auth_plugin_iface plugin_auth_oidc =
{
    {
        {
