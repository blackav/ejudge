...
enum { REGISTERED_NUMBER = 2 };

struct notify_redis_streams_plugin_data
{
...
extern struct notify_plugin_iface plugin_notify_redis_streams;

static struct common_plugin_data *
init_func(void)
{
    struct notify_redis_streams_plugin_data *state = NULL;
    XCALLOC(state, 1);
    state->b.vt = &plugin_notify_redis_streams;
    return &state->b.b;
}
...
static int
parse_passwd_file(
        struct notify_redis_streams_plugin_data *nrpd,
        const unsigned char *file)
{
...
{
    struct notify_redis_streams_plugin_data *nrpd = (struct notify_redis_streams_plugin_data*) data;
...
{
    __attribute__((unused)) struct notify_redis_streams_plugin_data *nrpd = (struct notify_redis_streams_plugin_data*) data;
        ...
        const unsigned char *message)
{
    ...
    if (nrpd->show_queries) {
        info("REDIS: (%s)(%s)(%s)(%s)(%s)", pub_args[0], pub_args[1], pub_args[2], pub_args[3], pub_args[4]);
    }
...
struct notify_plugin_iface plugin_notify_redis_streams =
{
    {
        {
