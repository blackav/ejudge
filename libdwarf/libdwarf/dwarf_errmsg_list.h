...
"DW_DLE_DEBUG_VARNAMES_NULL (165) .debug_varnames section present "
    "but elf_getdata() failed or section is zero-length",
...
"DW_DLE_LOC_BAD_TERMINATION (230) the last location operator "
    "in an expression is missing some associated data, "
    "an operator ended too soon",
...
"DW_DLE_LINE_NUMBER_HEADER_ERROR (235), a line number program "
    "header seems incomplete (perhaps the header_length is wrong?).",
...
"DW_DLE_SECTION_SIZE_ERROR(435) Corrupt Elf. Section size: "
    "greater than file size or not a multiple of section entry size",
...
"DW_DLE_MIXING_SPLIT_DWARF_VERSIONS(448) DWARF5 header "
    "signature and DWARF4 DW_AT_[GNU]_dwo_id "
    "present in one CU header/die. Corrupt Dwarf.",
...
"DW_DLE_NULL_ARGS_DWARF_ADD_PATH(464) either Dwarf_Debug or "
    "file_path argument to dwarf_add_file_path is NULL.",
...
"DW_DLE_DUPLICATE_GNU_DEBUG_PUBTYPES(475) Duplicated section "
    ".debug_gnu_pubtypes",
"DW_DLE_DEBUG_SUP_STRING_ERROR(476) The string in .debug_sup head "
