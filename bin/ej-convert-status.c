        ...
        int force_from_mode)
{
    ...
    struct statusdb_state *old_sdb_state = NULL;
    struct statusdb_state *new_sdb_state = NULL;
    ...
    const unsigned char *current_plugin = global->status_plugin;
    if (!current_plugin || !*current_plugin) {
        current_plugin = ejudge_config->default_status_plugin;
    }
    if (!current_plugin || !*current_plugin) {
        current_plugin = "file";
    }
    ...
    new_sdb_state = statusdb_open(ejudge_config, cnts, global, to_plugin, 0, 0);
    if (!new_sdb_state) {
        fprintf(stderr, "contest %d failed to load plugin %s\n",
